...
int** mm_2d_distributed(int** matrix_a, int** matrix_b, const Dimensions &dims);
...
int main(int argc, char* argv[]) {
  ...
  int** matrix_2d = mm_2d_distributed(matrix_a, matrix_b, dims);
  int** matrix_sequential = mm_sequential(matrix_a, matrix_b, dims);
  // mm_sequential(matrix_a, matrix_b, dims);

  // mm_1d_distributed(matrix_a, matrix_b, dims);

  if (r == 0) {
    verify(matrix_2d, matrix_sequential, dims.m, dims.q);
    print_matrix(matrix_2d, dims.m, dims.q);
    print_matrix(matrix_sequential, dims.m, dims.q);
  }
...
int** mm_2d_distributed(int** matrix_a, int** matrix_b, const Dimensions &dims) {
  ...
  for (int t = 0; t < root_p; t++) {
    ...
    for (int g = 1; g < p; g++) {
      ...
      if (r == 0) {
        if ((received_s - t + root_p) % root_p == received_r) {     
...
//   int** sequential_matrix = mm_sequential(matrix_a, matrix_b, dims);
...
//   if (r == 0) {
//     // int** control_matrix = mm_sequential(matrix_a, matrix_b, dims);
//     verify(result_matrix, sequential_matrix, m, q);

//   }

//   for (int s = 0; s < m; s++) {
//     delete [] sequential_matrix[s];
//   }
